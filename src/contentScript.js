...
if (!window.__passgenContentScriptLoaded) {
    ...
    function triggerFieldEvents(field) {
        const keyboardOpts = { bubbles: true, cancelable: true, key: 'Unidentified' };
        const eventOpts = { bubbles: true, cancelable: true };

        const hasKeydownHandler = triggerIfHandlerExists(field, 'keydown', KeyboardEvent, keyboardOpts);
        const hasKeypressHandler = triggerIfHandlerExists(field, 'keypress', KeyboardEvent, keyboardOpts);
        const hasKeyupHandler = triggerIfHandlerExists(field, 'keyup', KeyboardEvent, keyboardOpts);
        const hasChangeHandler = triggerIfHandlerExists(field, 'change', Event, eventOpts);
        const hasBlurHandler = triggerIfHandlerExists(field, 'blur', Event, { bubbles: true });

        const hasInputHandler = triggerIfHandlerExists(field, 'input', Event, eventOpts);
        if (hasInputHandler) {
    ...
    document.addEventListener('contextmenu', (e) => {
        ...
        if ((target instanceof HTMLInputElement && (target.type === 'password' || target.type === 'text')) ||
